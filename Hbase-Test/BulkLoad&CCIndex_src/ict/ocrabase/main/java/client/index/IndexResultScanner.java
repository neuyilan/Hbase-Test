...
public class IndexResultScanner {
	...
	public IndexResultScanner(List<SingleScanner> scanners, byte[][] resultColumns, int resultBufferSize, float loadFactor) {
		...
		//System.out.println("IndexResultScanner is started!");
	...
	/**
	 * Get the next result.
	 * <p> If current result buffer size is less than MaxSize*LoadFactor and there are stopped scanners,
	 * the stopped scanners will be restart automatically.
	 * 
	 * @return	if there is any result in the {@link #resultBuffer}, return one,
	...
	private void addFinished(String scannerName){
		synchronized(finishedLock){
			...
			if(LOG.isDebugEnabled()){
				...
				//System.out.println(scannerName+ "-"+s.getId()+" is added to finished scanner!");
			}
	...
	/**
	 * Get the number of results which have already been took out from result buffer with 
	...
	static abstract class SingleScanner extends Thread{
		...
		public SingleScanner(Scan scan, byte[][] resultColumns, HTable table) throws IOException{
			...
			//System.out.println("single scanner scan"+scan.toString());
			//Configuration newconf = HBaseConfiguration.create();
			//HTable newtable = new HTable(newconf, "t1-f1_c1");
			//Scan newscan =new Scan(Bytes.toBytes("20020"),Bytes.toBytes("20990"));
			//Scan nscan = new Scan(scan.getStartRow(),scan.getStopRow());
			//String sss = scan.toString();
			//ResultScanner newscanner = table.getScanner(scan);
			
		//	for(Result r:newscanner){
			//	String ss = Bytes.toString(r.getRow());
	//			System.out.println(ss);
		...
		public void restartScan(){
			if(finished==false && scanning==false){
				LOG.debug("Try to restart "+this.getName()+"-"+getId()+" for the "+indexResultScanner.restartTimes+"th times");
				//System.out.println("Try to restart "+this.getName()+"-"+getId()+" for the "+indexResultScanner.restartTimes+"th times");
				...
				try {
					...
					//System.out.println("Try to restart "+this.getName()+"-"+getId()+" failed!"+e.toString());
				}
				
			}else{
				LOG.warn(this.getName()+"-"+getId()+ " restart is ignored, because finished=" + finished
						+ ",scanning=" + scanning);
			}
		}
		...
		public void run(){
			...
			while(!finished){
				...
				if(!finished){
					...
					LOG.debug(this.getName()+"-"+getId()+" finish scanning,"+" total records:"+ count);
				}
			}
	...
	static class IndexSingleScanner extends SingleScanner{
		...
		public IndexSingleScanner(Scan scan, Range[] range, int flag,byte[][] resultColumns, HTable table, IndexKeyGenerator keyGen,
				IndexSpecification indexSpec, boolean containAll, byte[] mainTableName, FilterList list,int maxGets) throws IOException {
			super(scan, resultColumns, table);
			this.range=range;
			this.flag=flag;
			this.kegGen=keyGen;
			...
			if(containAll==false){
				...
				if(indexSpec.getIndexType()==IndexType.SECONDARYINDEX
						|| (indexSpec.getIndexType()==IndexType.IMPSECONDARYINDEX &&
						indexSpec.getAdditionMap().size()==0)
						||(resultColumns != null && resultColumns.length == 1 && flag >= 0 && 
		...
		public void run(){
			...
			//System.out.println(this.getName()+"-"+getId()+" start scanning, startkey:"
			//		+ Bytes.toStringBinary(scan.getStartRow()) + ", endkey:"
				//	+ Bytes.toStringBinary(scan.getStopRow()));
			...
			while(!finished){
					...
					while((rs=resultScanner.next())!=null && scanning){
						...
						if(existKey){
							if(range[keyflag].getStartValue()!=null){
								if(range[keyflag].getStartType()==CompareOp.GREATER){
									if(Bytes.compareTo(tempkey[0],range[keyflag].getStartValue())<=0){continue;}
							...
							if(range[keyflag].getEndValue()!=null){
								if(range[keyflag].getEndType()==CompareOp.LESS){
									if(Bytes.compareTo(tempkey[0],range[keyflag].getEndValue())>=0){continue;}
						...
						if(range[flag].getStartValue()!=null){
							if(range[flag].getStartType()==CompareOp.GREATER){
								if(Bytes.compareTo(tempkey[1],range[flag].getStartValue())<=0){continue;}
						...
						if(range[flag].getEndValue()!=null){
							if(range[flag].getEndType()==CompareOp.LESS){
								if(Bytes.compareTo(tempkey[1],range[flag].getEndValue())>=0){continue;}
						...
						if(containAll){
							templist=new TreeSet<KeyValue>(KeyValue.COMPARATOR);
								...
								for(KeyValue kv:rs.raw()){
									tempkv=new KeyValue(tempkey[0],kv.getFamily(),kv.getQualifier(),kv.getTimestamp(),kv.getValue());
									if(omitkv){continue;}	//omit kv whose qualifier is null
									templist.add(tempkv);
								}
							
							tempkv=new KeyValue(tempkey[0],indexColumn[0],indexColumn[1],tempkv.getTimestamp(),tempkey[1]);
							templist.add(tempkv);
							...
							threadPool[poolCounter]=new GetThread(ftlist,resultColumns,tables.get(poolCounter),tempkey[0]);
							threadPool[poolCounter].setName("Get-"+poolCounter);
							threadPool[poolCounter].setDaemon(true);
							threadPool[poolCounter].start();
							poolCounter++;
							
							if(poolCounter==maxGetsPerScanner){
								for(int i=0;i<maxGetsPerScanner;i++){
									threadPool[i].join();
				...
				if(!finished){
					scanning=false;
					LOG.debug(this.getName()+"-"+getId()+" stop scanning, waiting to be restarted!");
					//System.out.println(this.getName()+"-"+getId()+" stop scanning, waiting to be restarted!");
					indexResultScanner.addStopped(getName());
					LOG.debug(this.getName()+"-"+getId()+" is waked up, restart scan!");
					//System.out.println(this.getName()+"-"+getId()+" is waked up, restart scan!");
				}
				
				if(finished){
					indexResultScanner.addFinished(getName());						
					LOG.debug(this.getName()+"-"+getId()+" finish scanning,"+" total records:"+ count);
					//System.out.println(this.getName()+"-"+getId()+" finish scanning,"+" total records:"+ count);
				}
			}
